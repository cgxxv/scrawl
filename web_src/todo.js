...
const Todo = () => {
  ...
  const handleTodo = useCallback(async (e) => {
    try {
      if (e.key === 'Enter') {
        ...
        if (!id && !status) {
          const res = await axiosWrapper(`/api/todo`, {method: "POST", data: {title}});
          setTodoList((prevTodoList) => {
          ...
          await axiosWrapper(`/api/todo/update`, {method: "PUT", data: {title, status, id}});
  ...
  const toggleAllCheck = useCallback( (e) => {
    ...
    if (e.target.className === "selected") {
      e.target.className = "";
      lis.map((li) => {li.checked = false});
    } else {
      e.target.className = "selected";
      lis.map((li) => {li.checked = true});
    }
  }, [todoList]);
    ...
    <>
      <section className="todoapp">
                ...
                <li key={todo.id} className={todo.status === "Completed" ? "completed" : ""}>
                  <div className="view">
                    <input
                      className="toggle"
                      type="checkbox"
                      defaultChecked={todo.status === "Completed" || todo.status === "Active"}
                      onChange={toggleStatus(todo.id)}
                      data-id={todo.id}
                    />
                    <label onDoubleClick={handleUpdate(todo.id, todo.status)}>{todo.title}</label>
        ...
        <footer className="footer">
          ...
          <button className="clear-completed" onClick={handleActive}>Clear completed</button>
        </footer>
      </section>
